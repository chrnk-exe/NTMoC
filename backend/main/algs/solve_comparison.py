...
class SolveComparison:
    ...
    def solve(self):
        return self.solve_comparison()

    def solve_comparison(self):
        # print()
        # amgcd = np.gcd(a, m)
        ...
        #     b = b // amgcd
        #     m = m // amgcd
        m1, m2 = factorization(self.m)
        if not is_mutsimple(m1, m2):
            return [f'Unable to fact({self.m})']
        solution = [
            f"{self.a}x={self.b}(mod {m1})",
            f"{self.a}x={self.b}(mod {m2})"
        ]
        a1 = add_double(self.a, 1, m1, True)
        a2 = add_double(self.a, 1, m2, True)
        b1 = add_double(self.b, 1, m1, True)
        b2 = add_double(self.b, 1, m2, True)

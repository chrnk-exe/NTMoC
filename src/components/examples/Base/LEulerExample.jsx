  ...
  return (
    ...
    sx={{width: 1}} fontSize={17} gap={0.5}>
        ...
        <OutlinedFormula
          ...
          formula={'L(m) = M(\\phi(p_i^{a_i}), i \\in \\overline{1, n}))'}
          aftertext={'где $m$ - число, $M$ - Наименьшее общее кратное, $\\phi$ - Фи-функция Эйлера, а $p_i^{a_i}$ - разложение $m$ на простые множители. В случае, если m = 1, то значение L-функции равно 1'}
        />
    ...
    <SimpleTextFormula>{'$L(44) = M(\\phi(2^2), \\phi(11))$'}</SimpleTextFormula>
    <SimpleTextFormula>{'$\\phi(2^2) = 2^2 - 2^{2-1} = 4 - 2 = 2$  (по формуле $\\phi(p^n) = p^n - p^{n-1}$)'}</SimpleTextFormula>
    <SimpleTextFormula>{'$\\phi(11) = 10$'}</SimpleTextFormula>
